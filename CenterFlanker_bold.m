...
%%%%%%%%%%%%%
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% prompt={'姓名','性别（男填1，女填2）','年龄','主要药物类别'};
% dlg_line='被试信息';
% num_lines = 1;
% para=inputdlg(prompt,dlg_line,num_lines);
% %%
% ID=char(para{1,1});
%runNum=input('runNum锛');
%%
ID=input('SubName：','s');
...
Screen('Preference', 'SkipSyncTests', 1);
leftKey=KbName('n');% '2#' left
rightKey=KbName('m');% '1$' right
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% get screen
...
arrow_ground{1,1} = uint8(ones(height, width,3)*LUMINNACE);
arrow_ground{1,2} = uint8(ones(height, width,3)*LUMINNACE);
...
%% make cue
Cue_ground=uint8(ones(gr_x,gr_y,3)*LUMINNACE);
for m=1:n
    if m<=5
        line_x(m)=gr_x/2;
...
%     else
%         Selcet_lo{i}=[lo10 lo11 lo12 lo13 lo14 lo15 lo16 lo17 lo18 ];
%     end
% end
...
% array1=repmat(1:9,1,TrialNum);
dis_order=randperm(TrialNum);
...
for t=1:TrialNum
 ...
 if tar_type(t)<=(2*TrialNum)/3%80trials been cued
   loc=lo(tar_type(t));
...
for trial=1:TrialNum
   %%  fixation ISI
    ...
    Screen('Flip',w);
    while GetSecs<t+ISI_time;  %125ms锛岋紙鍘熸潵鏄?.1锛岀粨鍚坈ue鍙?.5锛岃繖涓彉0.5锛?
        ;
    end
%     %% PreCue present
%     t_cue=GetSecs;
%     Screen('DrawTexture',w,Cue_arrow);
%     Screen('Flip',w);
%     while GetSecs<t_cue+0.1;
%         ;
%     end
    %% 
    %%  Target
...
%     Tar_t=GetSecs;
   while GetSecs-t_begin<0.4;
         ...
         if  (~a)&&(keyCode(leftKey)||keyCode(rightKey)||keyCode(EscapeKey))
                    a=1;
                    if keyCode(rightKey)
                        result(4,trial)=1;%right
                    elseif keyCode(leftKey)
                        result(4,trial)=2;% left
                    end
                    if keyCode(EscapeKey)
                        Screen('Closeall');
                    end
...
%     
    ...
    Screen('DrawTexture',w,att,BackGRect);
    Screen('Flip',w);
    while GetSecs<rand(1)*(1-0.5)+1.5+t;  %1.5-2s(0.5鍒?绉掞級
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Block=num2str(mm);
FileName=['Bold_Center_drug' '_' ID '_' Block];
sts.All_ACC=ALL_ACC;
sts.AE=AE_sum;
sts.iAE=iAE_sum;
sts.O_AE=O_AE_sum;
...
sts.NIVACCM=NIVACC;
sts.result=result;
Name=['Bold_CF_flankAnalysis_result' '_' ID '_' Block];
