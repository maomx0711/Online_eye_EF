...
outputDir = 'D:\project total\Excitve function\Bold_EF\ANA_BoldCF';       % 结果输出目录
...
for run=1:RunNum
        ...
        sprintf('Bold_Center_drug_%s_%d.mat', subname, run));
...
for trial=1:TrialNum
    ...
    [im0(run) in0(run)]=size(RTivn0);[im1(run) in1(run)]=size(RTivn1);[im2(run) in2(run)]=size(RTivn2);[im3(run) in3(run)]=size(RTivn3);[im4(run) in4(run)]=size(RTivn4);
    ...
    [nm(run) nn(run)]=size(RTnvn); [inm(run) inn(run)]=size(RTinvn);
    %%%%%%%%%%%%%
    if run==1
        ...
        NRT_run(1:nm(run),1)=RTnvn(1:nm(run),1);

        %%%%%%%%%
        RTivn0_run(1:im0(run),1)=RTivn0(1:im0(run),1);
        RTivn1_run(1:im1(run),1)=RTivn1(1:im1(run),1);
        RTivn2_run(1:im2(run),1)=RTivn2(1:im2(run),1);
        RTivn3_run(1:im3(run),1)=RTivn3(1:im3(run),1);
        RTivn4_run(1:im4(run),1)=RTivn4(1:im4(run),1);
        INRT_run(1:inm(run),1)=RTinvn(1:inm(run),1);
    elseif run>1
        ...
        NRT_run(sum(nm(1:run-1))+1:sum(nm(1:run)),1)=RTnvn(1:nm(run),1);
        %%%%%%
        RTivn0_run(sum(im0(1:run-1))+1:sum(im0(1:run)),1)=RTivn0(1:im0(run),1);
        RTivn1_run(sum(im1(1:run-1))+1:sum(im1(1:run)),1)=RTivn1(1:im1(run),1);
        RTivn2_run(sum(im2(1:run-1))+1:sum(im2(1:run)),1)=RTivn2(1:im2(run),1);
        RTivn3_run(sum(im3(1:run-1))+1:sum(im3(1:run)),1)=RTivn3(1:im3(run),1);
        RTivn4_run(sum(im4(1:run-1))+1:sum(im4(1:run)),1)=RTivn4(1:im4(run),1);
        INRT_run(sum(inm(1:run-1))+1:sum(inm(1:run)),1)=RTinvn(1:inm(run),1);
    end
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% sum run
...
dataTable1 = cell2table(dataCell(2:end,:), 'VariableNames', dataCell(1,:));
 ...
 matFile = fullfile(outputDir, sprintf('Bold_CF_%s__fixsubresult.mat', subname));
excelFile = fullfile(outputDir, sprintf('Bold_CF_%s_report.xlsx', subname));
